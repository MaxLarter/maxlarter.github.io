...
```{r, include=FALSE}
source("cv/citation.R", local = knitr::knit_global())
# or sys.source("your-script.R", envir = knitr::knit_global())
```

```{r, include=FALSE}
# Load csv with position info
position_data <- read_csv('cv/CV_larter_short.csv') %>% 
  mutate_all(fill_nas) %>% 
...
- data analysis, stats, modelling, GIS
(R, python, git, SAS)
...
```{r, include=FALSE}
# Load csv with position info
position_data2 <- read_csv('cv/CV_larter.csv') %>% 
  mutate_all(fill_nas) %>% 
  arrange(order, desc(end)) %>% 
  mutate(id = 1:n()) %>% 
  nest(data = c(-id, -section))
