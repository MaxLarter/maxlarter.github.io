...
# set_scholar_mirror(mirror = "https://g88.i-research.edu.eu.org/ggxs")

#get_scholar_resp("https://scholar.google.com/citations?user=-WJKy_EAAAAJ&hl=en")
...
p <- ggplot(cites, aes(cites, year)) + 
    ...
    geom_text2(aes(label=cites, subset = cites > 500), hjust=1.1, size=6) + 
    labs(caption = "data from Google Scholar", size=6) +
