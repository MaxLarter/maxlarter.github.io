...
#    author=str_replace_all(author, " (\\S) ", "\\1 "),
    ...
    map(function(x){
      x <- x %>%
        glue_data('<tr><td width="100%">{author} ({year}) <a href="https://scholar.google.com/scholar?oi=bibs&cluster={cid}&btnI=1&hl=en" target="_blank">{title}</a>, {journal}, {number}, Cited {cites} times</td></tr>') %>%
        str_replace_all("(, )+</p>", "</p>") %>%
        char2html()
